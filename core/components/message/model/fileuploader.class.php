...
class fileUploader{
    ...
    protected $allow_type = array('pdf','mp3','mp4');// default is images
    /**
     * @param (object) $modx
     */
    public $modx = NULL;
    ...
    public function checkFile($file, $required=false){
        ...
        $this->allow_file_types[$file]['file_ext'] = $file_ext;
        # size
        ...
        if ( !in_array($file_ext, $allow_array) || !$this->_validFileType($file_ext, $_FILES[$file]['type']) ) {
            ...
            if( $file_ext == 'rtf' && $_FILES[$file]['type'] == 'application/msword' ) {
                ...
                //echo $this->error_array[$file].__LINE__;
                return FALSE;
            }
        }
    ...
    public function moveFile($file, $destination, $new_name) { //file is the name of the input feild, destination is full path with name
        if ( $this->isValid() && isset($this->allow_file_types[$file]['file_ext']) ) {
            $org_file = $_FILES[$file]['tmp_name'];
            $new_file = $destination.$new_name.'.'.$this->allow_file_types[$file]['file_ext'];
    ...
    public function fileExt($file) {
        return $this->allow_file_types[$file]['file_ext'];
    }
